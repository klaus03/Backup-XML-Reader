...
Das ist der Index des aktuellen Branches (macht nur Sinn wenn {filter => 5} gesetzt wurde).

=item rval

Das ist der Wert des aktuellen Branches (macht nur Sinn wenn {filter => 5} gesetzt wurde).

=item rvalue

Das ist eine Referenz (auf einen Scalar-Wert oder auf ein Array) fE<uuml>r den aktuellen Branch.
(macht nur Sinn wenn {filter => 5} gesetzt wurde). rvalue is schneller als, aber nicht so einfach
zu benutzen wie, die E<auml>hnliche funktion rval (mit rvalue muss man die De-Referenzierung selber
ausfE<uuml>hren.

=item rstem

Das ist der Stamm des aktuellen Branches (macht nur Sinn wenn {filter => 5} gesetzt wurde).
...
=head2 XML-Parsen mit {filter => 5}

Sie kE<ouml>nnen {filter => 5} und regulE<auml>re AusdrE<uuml>cke lombinieren:
