...
C'est la tige de la branche en cours (cette fonction est valide uniquement avec {filter => 5}).
...
Avec option {filter => 5}, on spE<eacute>cifie une (ou plusieurs) racines ("roots"), chaque racine a une collection
...
On peut combiner {filter => 5} avec des expressions rE<eacute>guliE<egrave>res pour parser les donnE<eacute>es XML:
