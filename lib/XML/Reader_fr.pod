...
C'est la valeur de la branche en cours (cette fonction est valide uniquement avec {filter => 5}).
...
mais moins simple E<agrave> utiliser que la fonction rval (avec rvalue vous devez faire votre
...
=head2 Option {filter => 2} ou {mode => 'attr-bef-start'}
...
=head2 Option {filter => 3} ou {mode => 'attr-in-hash'}
...
le module L<XML::Writer>. Voici un programme qui utilise L<XML::Writer> pour E<eacute>crire du XML:
...
Voici le rE<eacute>sultat de L<XML::Writer>:
...
=head2 Option {filter => 4} ou {mode => 'pyx'}
...
=head2 Option {filter => 5} ou {mode => 'branches'}
...
identifiable par $rdr->rx == 2, et les donnE<eacute>es pour la quatriE<egrave>me racine ('p')
sont identifiable par $rdr->rx == 3.
...
Nous pouvons E<eacute>galement utiliser la fonction $rdr->rval pour obtenir les mE<ecirc>mes donnE<eacute>es:
...
=head2 Parser l'exemple XML avec l'option {filter => 2} ou {mode => 'attr-bef-start'}
...
=head2 Parser l'exemple XML avec l'option {filter => 3} ou {mode => 'attr-in-hash'}
...
=head2 Parser l'exemple XML avec l'option {filter => 4} ou {mode => 'pyx'}
...
=head2 Parser l'exemple XML avec l'option {filter => 5} ou {mode => 'branches'}
